...
float rad2deg(float);
float findR(float, float);
int findA(float, float, float);
...
void main(){
  ...
  for (i = 0; i < 8; i++) r[i] = findR(x[i], y[i]);

  for (i = 0; i < 8; i++) a[i] = findA(x[i], y[i], r[i]);

  printf ("| Planets\t|\t  X\t|\t  Y\t|\t  R\t|\t  A |\n");
  printf ("-----------------------------------------------------------------------------\n");

  for (i = 0; i < 8; i++)
    printf ("| %s\t|\t%7.3f\t|\t%7.3f\t|\t%7.3f\t|\t%3d |\n", planets[i], x[i], y[i],r[i], a[i]);
}
...
int findA(float x, float y, float r){
  return y > 0 ? round(rad2deg(acos(x/r))) : round (360-rad2deg(acos(x/r))); 
}
